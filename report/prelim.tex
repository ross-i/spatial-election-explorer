...
Let there be $n$ voters and $m$ candidates.
...
\textbf{Borda.}
Each candidate's score is the sum of the number of candidates ranked below them across all ballots.
...
\begin{itemize}[topsep=0em]
  ...
  First-place votes are tallied.
  The $k$ candidates with the highest score are elected.
  ...
  reweight their supporters by a factor of $\frac{s-q}{s}$,
  ...
  the candidate with greater score is elected,
  and their support is redistributed before any other candidates are elected.

\end{itemize}

For all voting rules, ties are broken arbitrarily.
